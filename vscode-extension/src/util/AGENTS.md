# AGENTS.md — vscode-extension/src/util/
...
| File | What it does |
|------|-------------|
| `workspace.ts` | `getRepoRoot()` — returns the first workspace folder's `fsPath`, or `undefined` if no folder is open. Used by `extension.ts` during activation. |
