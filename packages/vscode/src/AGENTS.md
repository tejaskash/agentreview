...
| `core-bridge.ts` | Per-repo adapter wrapping core modules from `agentreview`. Methods: session CRUD, git ops (async), thread CRUD, anchor ops, export bundle generation. All wrapped in try/catch returning `undefined` on error, except `applyPatch` which re-throws. |
